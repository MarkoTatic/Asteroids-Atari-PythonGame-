    ...
    def moveSelf(self):
        ...
        if floor(self.init_x) <= -50 or floor(self.init_x) >= 623 or floor(self.init_y) <= -50 or floor(#vece dimenzije za metak jer bagguje nestoo ako se samnje , tj bug je u tome da se fakticki puca van scene
            ...
            if self.rocket_id == 1:#ako je metak od rakete 1 izbaci ga iz niza i unisti objekat
                ...
                Server.bulletsCollection3X[self.IDS] = 2222
                Server.bulletsCollection3Y[self.IDS] = 2222
                Server.rocket3_bulletsCounter = Server.rocket3_bulletsCounter - 1
            elif self.rocket_id == 4:
                Server.bulletsCollection4X[self.IDS] = 2222
                Server.bulletsCollection4Y[self.IDS] = 2222
                Server.rocket4_bulletsCounter = Server.rocket4_bulletsCounter - 1
            ...
            if self.rocket_id == 1:
                ...
                del Server.bulletsCollection3X[self.IDS]
                del Server.bulletsCollection3Y[self.IDS]
            elif self.rocket_id == 4:
                del Server.bulletsCollection4X[self.IDS]
                del Server.bulletsCollection4Y[self.IDS]
        else:
            self.move(self.init_x, self.init_y)
            ...
            if self.rocket_id == 1:#ako je metak od rakete 1 pomeri ga
                ...
                Server.bulletsCollection3X[self.IDS] = inttXXX
                Server.bulletsCollection3Y[self.IDS] = inttYYY
            elif self.rocket_id == 4:
                Server.bulletsCollection4X[self.IDS] = inttXXX
                Server.bulletsCollection4Y[self.IDS] = inttYYY
